...
// Implement the Dispatcher class here
Dispatcher::Dispatcher(unsigned int n, unsigned int t)
{
    numberOfRequests = n;
    time_interval = t;
    active_requests = 0;
    end_time = get_current_time_in_secs() + t;
}
...
// Implement the enable_request method here
bool Dispatcher::enable_request(){
  ...
  if (current_time >= end_time) {
      // The time interval has ended; reset active request count
      active_requests = 0;
  ...
  if (active_requests < numberOfRequests) {
      // Increment the active request count and allow the request
      active_requests++;
      return true;
  }

  // Maximum number of requests reached; reject the request
  return false;
}
